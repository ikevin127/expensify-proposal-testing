...
describe('Math Utils', () => {
    ...
    describe('round', () => {
        ...
        test('should round to specified decimal places', () => {
            ...
            expect(round(2.678, 1)).toBe(2.7);
            expect(round(2.674, 1)).toBe(2.7);
        });

        test('should handle zero decimal places explicitly', () => {
            expect(round(3.14159, 0)).toBe(3);
            expect(round(7.8, 0)).toBe(8);
            expect(round(-4.2, 0)).toBe(-4);
            expect(round(-4.8, 0)).toBe(-5);
        });

        test('should handle negative numbers', () => {
            expect(round(-3.14159, 2)).toBe(-3.14);
            expect(round(-3.678, 1)).toBe(-3.7);
        ...
        test('should handle very small decimal values', () => {
            expect(round(0.000123, 5)).toBe(0.00012);
            expect(round(0.000127, 5)).toBe(0.00013);
            expect(round(0.999999, 4)).toBe(1);
            expect(round(-0.000456, 5)).toBe(-0.00046);
        });

        test('should handle large decimal place counts', () => {
            expect(round(3.141592653589793, 10)).toBe(3.1415926536);
            expect(round(1.23456789, 8)).toBe(1.23456789);
            expect(round(1.999999999, 8)).toBe(2);
        });
        ...
        test('should handle very large numbers', () => {
            ...
            expect(round(999999.999, 1)).toBe(1000000);
            expect(round(-987654.321, 2)).toBe(-987654.32);
        });
        ...
        test('should throw error for negative decimal places', () => {
            ...
            expect(() => round(5.67, -2)).toThrow('Number of decimal places cannot be negative');
            expect(() => round(0, -5)).toThrow('Number of decimal places cannot be negative');
        });

        test('should throw error for non-integer decimal places', () => {
            expect(() => round(3.14, 1.5)).toThrow('Number of decimal places must be an integer');
            expect(() => round(5.67, 2.3)).toThrow('Number of decimal places must be an integer');
            expect(() => round(0, 0.1)).toThrow('Number of decimal places must be an integer');
        });
        ...
        test('should preserve precision for exact decimal representations', () => {
            expect(round(0.1 + 0.2, 1)).toBe(0.3); // Handles floating point precision issues
            expect(round(1.005, 2)).toBe(1);
            expect(round(2.055, 2)).toBe(2.06);
        });
    });
});
