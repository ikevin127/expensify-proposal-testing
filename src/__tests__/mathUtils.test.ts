...
describe('Math Utils', () => {
    ...
    describe('slope', () => {
        test('should calculate positive slope', () => {
            expect(slope(0, 0, 2, 4)).toBe(2);
            expect(slope(1, 1, 3, 5)).toBe(2);
            expect(slope(-1, -2, 1, 2)).toBe(2);
        });

        test('should calculate negative slope', () => {
            expect(slope(0, 4, 2, 0)).toBe(-2);
            expect(slope(1, 5, 3, 1)).toBe(-2);
            expect(slope(-1, 2, 1, -2)).toBe(-2);
        });

        test('should calculate zero slope for horizontal lines', () => {
            expect(slope(0, 5, 10, 5)).toBe(0);
            expect(slope(-5, 3, 5, 3)).toBe(0);
            expect(slope(1, -2, 100, -2)).toBe(0);
        });

        test('should calculate fractional slopes', () => {
            expect(slope(0, 0, 3, 1)).toBeCloseTo(1/3, 5);
            expect(slope(1, 2, 4, 3)).toBeCloseTo(1/3, 5);
            expect(slope(0, 0, 2, 3)).toBe(1.5);
        });

        test('should handle negative coordinates', () => {
            expect(slope(-2, -1, -1, 1)).toBe(2);
            expect(slope(-3, 2, -1, -4)).toBe(-3);
        });

        test('should throw error for vertical lines', () => {
            expect(() => slope(5, 0, 5, 10)).toThrow('Cannot calculate slope for vertical line (undefined slope)');
            expect(() => slope(-2, 3, -2, 7)).toThrow('Cannot calculate slope for vertical line (undefined slope)');
            expect(() => slope(0, 0, 0, 5)).toThrow('Cannot calculate slope for vertical line (undefined slope)');
        });

        test('should handle identical points', () => {
            expect(() => slope(3, 4, 3, 4)).toThrow('Cannot calculate slope for vertical line (undefined slope)');
        });
    });

    describe('hypotenuse', () => {
        test('should calculate hypotenuse correctly', () => {
            expect(hypotenuse(3, 4)).toBe(5);
            expect(hypotenuse(5, 12)).toBe(13);
            expect(hypotenuse(8, 15)).toBe(17);
        });

        test('should handle decimal sides', () => {
            expect(hypotenuse(1.5, 2)).toBeCloseTo(2.5, 5);
            expect(hypotenuse(2.1, 2.8)).toBeCloseTo(3.5, 5);
        });

        test('should handle zero sides', () => {
            expect(hypotenuse(0, 5)).toBe(5);
            expect(hypotenuse(7, 0)).toBe(7);
            expect(hypotenuse(0, 0)).toBe(0);
        });

        test('should handle equal sides', () => {
            expect(hypotenuse(1, 1)).toBeCloseTo(Math.sqrt(2), 5);
            expect(hypotenuse(3, 3)).toBeCloseTo(3 * Math.sqrt(2), 5);
        });
        ...
        test('should handle large numbers', () => {
            expect(hypotenuse(300, 400)).toBe(500);
            expect(hypotenuse(1000, 1000)).toBeCloseTo(1000 * Math.sqrt(2), 5);
        });
    });

    describe('triangleArea', () => {
        test('should calculate area using Heron\'s formula', () => {
            expect(triangleArea(3, 4, 5)).toBe(6); // Right triangle
            expect(triangleArea(5, 5, 8)).toBe(12); // Isosceles triangle
            expect(triangleArea(6, 8, 10)).toBe(24); // Scaled 3-4-5 triangle
        });

        test('should handle equilateral triangles', () => {
            const side = 4;
            const expectedArea = (Math.sqrt(3) / 4) * side * side;
            expect(triangleArea(side, side, side)).toBeCloseTo(expectedArea, 5);
        });

        test('should handle decimal side lengths', () => {
            expect(triangleArea(1.5, 2, 2.5)).toBe(1.5); // Scaled 3-4-5 triangle
            expect(triangleArea(2.1, 2.8, 3.5)).toBeCloseTo(2.94, 2);
        });

        test('should throw error for non-positive sides', () => {
            expect(() => triangleArea(0, 4, 5)).toThrow('All side lengths must be positive');
            expect(() => triangleArea(3, -4, 5)).toThrow('All side lengths must be positive');
            expect(() => triangleArea(-3, -4, -5)).toThrow('All side lengths must be positive');
        });

        test('should throw error for invalid triangles (triangle inequality)', () => {
            expect(() => triangleArea(1, 2, 5)).toThrow('Invalid triangle: sides do not satisfy triangle inequality');
            expect(() => triangleArea(1, 1, 3)).toThrow('Invalid triangle: sides do not satisfy triangle inequality');
            expect(() => triangleArea(2, 3, 10)).toThrow('Invalid triangle: sides do not satisfy triangle inequality');
        });
    ...
    describe('convertBase', () => {
        test('should convert between common bases', () => {
        ...
        test('should convert to and from octal', () => {
            expect(convertBase('17', 8, 10)).toBe('15');
            expect(convertBase('15', 10, 8)).toBe('17');
            expect(convertBase('377', 8, 16)).toBe('FF');
        });

        test('should handle base 36', () => {
            expect(convertBase('Z', 36, 10)).toBe('35');
            expect(convertBase('35', 10, 36)).toBe('Z');
            expect(convertBase('10', 36, 10)).toBe('36');
        });

        test('should handle lowercase and uppercase', () => {
            expect(convertBase('abc', 16, 10)).toBe('2748');
            expect(convertBase('ABC', 16, 10)).toBe('2748');
            expect(convertBase('2748', 10, 16)).toBe('ABC');
        });

        test('should handle zero', () => {
            expect(convertBase('0', 2, 10)).toBe('0');
            expect(convertBase('0', 10, 16)).toBe('0');
            expect(convertBase('0', 36, 2)).toBe('0');
        });

        test('should throw error for invalid bases', () => {
            expect(() => convertBase('10', 1, 10)).toThrow('Base must be between 2 and 36');
            expect(() => convertBase('10', 37, 10)).toThrow('Base must be between 2 and 36');
            expect(() => convertBase('10', 10, 1)).toThrow('Base must be between 2 and 36');
            expect(() => convertBase('10', 10, 37)).toThrow('Base must be between 2 and 36');
        });

        test('should throw error for empty number string', () => {
            expect(() => convertBase('', 10, 2)).toThrow('Number string cannot be empty');
            expect(() => convertBase('   ', 10, 2)).toThrow('Number string cannot be empty');
        });

        test('should throw error for invalid number for base', () => {
            expect(() => convertBase('2', 2, 10)).toThrow('Invalid number for the specified base');
            expect(() => convertBase('G', 16, 10)).toThrow('Invalid number for the specified base');
            expect(() => convertBase('89', 8, 10)).toThrow('Invalid number for the specified base');
        });
    });

    describe('digitSum', () => {
        test('should calculate sum of digits for positive numbers', () => {
            expect(digitSum(123)).toBe(6); // 1 + 2 + 3
            expect(digitSum(456789)).toBe(39); // 4 + 5 + 6 + 7 + 8 + 9
            expect(digitSum(1000)).toBe(1); // 1 + 0 + 0 + 0
        });
        ...
        test('should handle negative numbers by using absolute value', () => {
            expect(digitSum(-123)).toBe(6); // |−123| = 123 → 1 + 2 + 3
            expect(digitSum(-456)).toBe(15); // |−456| = 456 → 4 + 5 + 6
            expect(digitSum(-7)).toBe(7); // |−7| = 7
        });

        test('should handle numbers with zeros', () => {
            expect(digitSum(1001)).toBe(2); // 1 + 0 + 0 + 1
            expect(digitSum(102030)).toBe(6); // 1 + 0 + 2 + 0 + 3 + 0
            expect(digitSum(505)).toBe(10); // 5 + 0 + 5
        });

        test('should handle large numbers', () => {
            expect(digitSum(999999)).toBe(54); // 9 × 6
            expect(digitSum(1234567890)).toBe(45); // 1+2+3+4+5+6+7+8+9+0
        });

        test('should handle repeated digits', () => {
            expect(digitSum(1111)).toBe(4); // 1 + 1 + 1 + 1
            expect(digitSum(777)).toBe(21); // 7 + 7 + 7
            expect(digitSum(2222222)).toBe(14); // 2 × 7
        });

        test('should handle edge cases', () => {
            expect(digitSum(10)).toBe(1); // 1 + 0
            expect(digitSum(100)).toBe(1); // 1 + 0 + 0
            expect(digitSum(9876543210)).toBe(45); // Sum of all digits 0-9
        });
    });
